...
class Ball:
    ...
    def move(self):
        ...
        if self.y - self.radius == 0:
            self.speed_y = -self.speed_y
        if self.x + self.radius == self.screen.get_width() and self.y + self.radius == self.screen.get_height():
            self.color = (self.red, self.green, self.blue)
        if self.x + self.radius == self.screen.get_width() and self.y - self.radius == 0:
            self.color = (self.red, self.green, self.blue)
        if self.x - self.radius == 0 and self.y + self.radius == self.screen.get_height():
            self.color = (self.red, self.green, self.blue)
        if self.x - self.radius == 0 and self.y - self.radius == 0:
            self.color = (self.red, self.green, self.blue)
        self.x += self.speed_x
        self.y += self.speed_y
...
def main():
    ...
    ball1 = Ball(screen, pygame.Color("Cyan"), 150, 150, 10, 10, 5)
